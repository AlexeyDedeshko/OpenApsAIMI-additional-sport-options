...
fun TemporaryTarget.lowValueToUnitsToString(units: GlucoseUnit, decimalFormatter: DecimalFormatter): String =
    if (units == GlucoseUnit.MGDL) {
        decimalFormatter.to0Decimal(this.lowTarget)
    } else {
        decimalFormatter.to1Decimal(this.lowTarget * Constants.MGDL_TO_MMOLL)
    }

fun TemporaryTarget.highValueToUnitsToString(units: GlucoseUnit, decimalFormatter: DecimalFormatter): String =
    if (units == GlucoseUnit.MGDL) {
        decimalFormatter.to0Decimal(this.highTarget)
    } else {
        decimalFormatter.to1Decimal(this.highTarget * Constants.MGDL_TO_MMOLL)
    }

fun TemporaryTarget.target(): Double = (this.lowTarget + this.highTarget) / 2
