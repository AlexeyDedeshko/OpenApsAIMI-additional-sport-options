...
class BolusWizard @Inject constructor(
    ...
    fun doCalc(
        ...
        val percentage = if (usePercentage) {
            totalPercentage
        } else {
            percentageCorrection.toDouble()
        }
        ...
        if (calculatedTotalInsulin >= 0) {
            ...
            //Should be updated after calcCorrectionWithConstraints and calcPercentageWithConstraints to have correct synthesis in WizardInfo
            if (usePercentage) {
    ...
    fun explainShort(): String {
        ...
        if (useTT && tempTarget != null) {
            val tt = if (tempTarget?.lowTarget == tempTarget?.highTarget) {
