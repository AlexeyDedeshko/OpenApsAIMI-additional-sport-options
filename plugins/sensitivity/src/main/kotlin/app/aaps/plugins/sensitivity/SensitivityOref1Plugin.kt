...
class SensitivityOref1Plugin @Inject constructor(
    ...
    override fun detectSensitivity(ads: AutosensDataStore, fromTime: Long, toTime: Long): AutosensResult {
        ...
        while (index < ads.autosensDataTable.size()) {
            ...
            while (hourSegment < deviationsHour.size) {
                ...
                if (autosensData.validDeviation) {
                    // брать данные не старее одного часа от toTime
                    if (autosensData.time > toTime - hoursDetection[hourSegment] * 60 * 60 * 1000L) {
                ...
                // если размер массива больше необходимого, то удаляем наиболее старые значения (с начала массива)
        ...
        while (hourUsed < deviationsHour.size) {
            ...
            val sens = profile.getIsfMgdl() // ISF
            aapsLogger.debug(LTag.AUTOSENS, "Records: $index   $pastSensitivity")

            // сортировать массив deviations для расчета процентиля
        ...
        val output = fillResult(
            ratioArray[key],
            current.cob,
            pastSensitivityArray[key],
            ratioLimitArray[key],
            sensResultArray[key] + comparison,
            deviationsHour[key].size
        )
