...
class MainMenuActivity : MenuListActivity() {
    ...
    override fun provideElements(): List<MenuItem> =
        ArrayList<MenuItem>().apply {
            if (!preferences.get(BooleanKey.WearControl)) {
                ...
                add(MenuItem(R.drawable.ic_target_activity, getString(R.string.menu_exercise_mode)))
    ...
    override fun doAction(position: String) {
        when (position) {
            ...
            getString(R.string.menu_exercise_mode)   -> startActivity(Intent(this, ExerciseModeActivity::class.java).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) })
